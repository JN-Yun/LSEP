...
class LabelEmbedder(nn.Module):
    ...
    def token_drop(self, labels, force_drop_ids=None, uncond_prob=0.9):
        """
        Drops labels to enable classifier-free guidance.
        """
        if force_drop_ids is None:
            drop_ids = torch.rand(labels.shape[0], device=labels.device) < self.dropout_prob 
        elif force_drop_ids == "uncond": # unconditioning linear improving
            drop_ids = torch.rand(labels.shape[0], device=labels.device) < uncond_prob
        else:
            drop_ids = force_drop_ids == 1
        labels = torch.where(drop_ids, self.num_classes, labels)
        return labels

    def forward(self, labels, train, force_drop_ids=None, uncond_prob=0.9):
        use_dropout = self.dropout_prob > 0
        if (train and use_dropout) or (force_drop_ids is not None):
            labels = self.token_drop(labels, force_drop_ids, uncond_prob)
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        num_classes=1000,
        use_cfg=False,
        random_crop_kwargs = [False, 16, 17],
        uncond_prob = 0.9,
        **block_kwargs, # fused_attn
        ...
        self.random_crop = random_crop_kwargs[0]
        self.crop_start = random_crop_kwargs[1]
        self.crop_end = random_crop_kwargs[2]
    ...
    def random_spatial_crop(self, x, crop_size, per_sample=False):
        """
        x: Tensor of shape (B, T, D), where T = t * t
        crop_size: int, s ≤ t, crop window size
        per_sample: bool, if True, each batch gets its own random crop
        Returns:
            cropped: Tensor of shape (B, S, D), where S = s * s
        """
        B, T, D = x.shape
        t = int(T ** 0.5)
        s = crop_size
        assert t * t == T, f"T={T} should be a perfect square."
        assert s <= t, f"crop_size s={s} must be equal to or smaller than t={t}."

        x = x.view(B, t, t, D)  # Reshape (B, T, D) -> (B, t, t, D)
        max_offset = t - s

        if per_sample: # Random crop for each sample
            top = torch.randint(0, max_offset + 1, (B,))
            left = torch.randint(0, max_offset + 1, (B,))
            cropped = torch.stack([
                x[b, top[b]:top[b]+s, left[b]:left[b]+s, :] for b in range(B)
            ], dim=0)  # shape: (B, s, s, D)
        else: # Shared crop for all batches
            top = torch.randint(0, max_offset + 1, ()).item()
            left = torch.randint(0, max_offset + 1, ()).item()
            cropped = x[:, top:top+s, left:left+s, :]  # shape: (B, s, s, D)

        cropped = cropped.reshape(B, s * s, D)  # shape: (B, S, D)
        return cropped
    ...
    def forward(self, x, t, y):
        ...
        # for linear probing
        if self.training:
            y_uncond = self.y_embedder(y, self.training, force_drop_ids='uncond', uncond_prob=self.uncond_prob)   
            c_uncond = t_embed + y_uncond                          
            x_uncond = x.clone()
            cal_uncond = True
            ...
            if self.training:
                if cal_uncond:
                    ...
                    if (i + 1) == self.encoder_depth:
                        ...
                        if self.random_crop:
                            window_size = torch.randint(self.crop_start, self.crop_end, ()).item()
                            zs = self.random_spatial_crop(zs, crop_size=window_size, per_sample=True)
                            zs = zs.float().mean(dim=1)  
                        else:
                            zs = zs.float().mean(dim=1)  
                        zs = self.classifier(zs)
                        cal_uncond = False
            else:
                zs = None
