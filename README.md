...
We propose Linear SEParability (LSEP) as a regularization method for training. By promoting the linear separability of intermediate layer representations using a linear probe (classifier), LSEP significantly improves both training efficiency and generative performance on flow-based transformer architectures (SiTs). Our approach achieves FID of **1.42** on the 256 × 256 and **1.66** on the 512 × 512 ImageNet dataset *without relying on
...
```
| ImageNet Size |$\qquad$ Model                 | Epoch               | FID (w/o CFG) | FID (w/ CFG) | CFG 
|:----:|--------------------|:-----------------:|:-------------:|:------------:| 
| 256  | SiT-XL (Baseline)     | 1400              | 8.3           | 2.06         |
|      | SiT-XL + **LSEP**     | **800**               | **5.7**           | **1.42**         | 1.75 [0, 0.625]
| 512  | SiT-XL (Baseline)     | 600               | -             | 2.62         | 
|      | SiT-XL + LSEP     | 240               | -             | 2.00         | 1.9 [0, 0.75]
|      | SiT-XL + **LSEP** (Fine tuning) | **100**     | -             | **1.66**         | 1.9 [0, 0.75]
